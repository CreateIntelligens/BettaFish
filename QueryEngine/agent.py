...
class DeepSearchAgent:
    ...
    def __init__(self, config: Optional[Settings] = None):
        ...
        logger.info(f"搜索工具集: TavilyNewsAgency (支持6種搜索工具)")
    ...
    def execute_search_tool(self, tool_name: str, query: str, **kwargs) -> TavilyResponse:
        ...
        Args:
            tool_name: 工具名稱，可選值：
                - "basic_search_news": 基礎新聞搜索（快速、通用）
                - "deep_search_news": 深度新聞分析
                - "search_news_last_24_hours": 24小時內最新新聞
                - "search_news_last_week": 本週新聞
                - "search_images_for_news": 新聞圖片搜索
                - "search_news_by_date": 按日期範圍搜索新聞
            query: 搜索查詢
            **kwargs: 額外參數（如start_date, end_date, max_results）
            
        Returns:
            TavilyResponse對象
        """
        logger.info(f"  → 執行搜索工具: {tool_name}")
        ...
        if tool_name == "basic_search_news":
            ...
            if not start_date or not end_date:
                raise ValueError("search_news_by_date工具需要start_date和end_date參數")
            return self.search_agency.search_news_by_date(query, start_date, end_date)
        else:
            logger.warning(f"  ⚠️  未知的搜索工具: {tool_name}，使用默認基礎搜索")
    ...
    def research(self, query: str, save_report: bool = True) -> str:
        ...
        try:
            ...
            logger.error(f"研究過程中發生錯誤: {str(e)} \n錯誤堆棧: {error_traceback}")
            raise e
    ...
    def _initial_search_and_summary(self, paragraph_index: int):
        ...
        search_tool = search_output.get("search_tool", "basic_search_news")  # 默認工具
        ...
        if search_tool == "search_news_by_date":
            ...
            if start_date and end_date:
                ...
                if self._validate_date_format(start_date) and self._validate_date_format(end_date):
                    ...
                    logger.info(f"  ⚠️  日期格式錯誤（應爲YYYY-MM-DD），改用基礎搜索")
                    logger.info(f"      提供的日期: start_date={start_date}, end_date={end_date}")
                    search_tool = "basic_search_news"
            else:
                logger.info(f"  ⚠️  search_news_by_date工具缺少時間參數，改用基礎搜索")
                search_tool = "basic_search_news"
    ...
    def _reflection_loop(self, paragraph_index: int):
        ...
        for reflection_i in range(self.config.MAX_REFLECTIONS):
            ...
            search_tool = reflection_output.get("search_tool", "basic_search_news")  # 默認工具
            ...
            if search_results:
                logger.info(f"    找到 {len(search_results)} 個反思搜索結果")
                for j, result in enumerate(search_results, 1):
                    date_info = f" (發佈於: {result.get('published_date', 'N/A')})" if result.get('published_date') else ""
    ...
    def _generate_final_report(self) -> str:
        ...
        try:
            ...
            logger.error(f"LLM格式化失敗，使用備用方法: {str(e)}")
