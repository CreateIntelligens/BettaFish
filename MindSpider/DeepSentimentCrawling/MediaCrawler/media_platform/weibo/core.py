...
class WeiboCrawler(AbstractCrawler):
    ...
    async def start(self):
        ...
        async with async_playwright() as playwright:
            ...
            if config.ENABLE_CDP_MODE:
                utils.logger.info("[WeiboCrawler] 使用CDP模式啓動瀏覽器")
                ...
                utils.logger.info("[WeiboCrawler] 使用標準模式啓動瀏覽器")
            ...
            if not await self.wb_client.pong():
                ...
                # 登錄成功後重定向到手機端的網站，再更新手機端登錄成功的cookie
    ...
    async def launch_browser_with_cdp(
        ...
        try:
            ...
            utils.logger.info(f"[WeiboCrawler] CDP瀏覽器信息: {browser_info}")

            return browser_context

        except Exception as e:
            utils.logger.error(f"[WeiboCrawler] CDP模式啓動失敗，回退到標準模式: {e}")
            # 回退到標準模式
            chromium = playwright.chromium
