...
class DouYinClient(AbstractApiClient):
    ...
    def __init__(
        self,
        timeout=60,  # 若開啓爬取媒體選項，抖音的短視頻需要更久的超時時間
        proxy=None,
        *,
        headers: Dict,
    ...
    async def get_aweme_all_comments(
        ...
        獲取帖子的所有評論，包括子評論
        :param aweme_id: 帖子ID
        :param crawl_interval: 抓取間隔
        :param is_fetch_sub_comments: 是否抓取子評論
        :param callback: 回調函數，用於處理抓取到的評論
        :param max_count: 一次帖子爬取的最大評論數量
        :return: 評論列表
        """
        result = []
        comments_has_more = 1
    ...
    async def resolve_short_url(self, short_url: str) -> str:
        """
        解析抖音短鏈接,獲取重定向後的真實URL
        Args:
            short_url: 短鏈接,如 https://v.douyin.com/iF12345ABC/
        Returns:
            重定向後的完整URL
        """
