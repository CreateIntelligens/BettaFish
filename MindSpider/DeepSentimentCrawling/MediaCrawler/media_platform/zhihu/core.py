...
class ZhihuCrawler(AbstractCrawler):
    ...
    async def start(self) -> None:
        ...
        async with async_playwright() as playwright:
            ...
            if config.ENABLE_CDP_MODE:
                utils.logger.info("[ZhihuCrawler] 使用CDP模式啓動瀏覽器")
                ...
                utils.logger.info("[ZhihuCrawler] 使用標準模式啓動瀏覽器")
            ...
            # 知乎的搜索接口需要打開搜索頁面之後cookies才能訪問API，單獨的首頁不行
            utils.logger.info(
                "[ZhihuCrawler.start] Zhihu跳轉到搜索頁面獲取搜索頁面的Cookies，該過程需要5秒左右"
            )
    ...
    async def get_creators_and_notes(self) -> None:
        ...
        for user_link in config.ZHIHU_CREATOR_URL_LIST:
            ...
            # 默認只提取回答信息，如果需要文章和視頻，把下面的註釋打開即可
    ...
    async def launch_browser_with_cdp(
        ...
        try:
            ...
            utils.logger.info(f"[ZhihuCrawler] CDP瀏覽器信息: {browser_info}")

            return browser_context

        except Exception as e:
            utils.logger.error(f"[ZhihuCrawler] CDP模式啓動失敗，回退到標準模式: {e}")
            # 回退到標準模式
            chromium = playwright.chromium
