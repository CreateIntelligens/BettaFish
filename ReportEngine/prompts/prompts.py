...
可用模板類型（推薦優先考慮「社會公共熱點事件分析報告模板」）：
- 企業品牌聲譽分析報告模板：適用於品牌形象、聲譽管理分析，當需要對品牌在特定週期內（如年度、半年度）的整體網絡形象、資產健康度進行全面、深度的評估與覆盤時，應選擇此模板。核心任務是戰略性、全局性分析。
...
- 社會公共熱點事件分析報告模板：當社會上出現與本機構無直接關聯，但已形成廣泛討論的公共熱點、文化現象或網絡流行趨勢時，應選擇此模板。核心任務是洞察社會心態，並評估事件與本機構的關聯性（風險與機遇）。
