...
class ForumHost:
    ...
    def __init__(self, api_key: str = None, base_url: Optional[str] = None, model_name: Optional[str] = None):
        ...
        Args:
            api_key: 論壇主持人 LLM API Key，如果不提供則從配置文件讀取
            base_url: 論壇主持人 LLM API 接口基礎地址，默認使用配置文件提供的SiliconFlow地址
        """
        self.api_key = api_key or settings.FORUM_HOST_API_KEY

        if not self.api_key:
            raise ValueError("未找到論壇主持人API密鑰，請在config.py中設置FORUM_HOST_API_KEY")
